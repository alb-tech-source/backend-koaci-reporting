import { z } from "zod";

export const presignCompanyDocumentSchema = z.object({
  company_id: z.uuid("Format company_id tidak valid"),
  file_name: z.string().trim().min(1).max(150),
  mime_type: z.string().trim().min(1).max(100),
  file_size_bytes: z.number().int().positive(),
});

export const createCompanyDocumentBodySchema = z.object({
  company_id: z.uuid("Format company_id tidak valid"),
  document_type: z.string().trim().min(2).max(100).optional(),
  document_name: z.string().trim().min(2).max(150),
  object_key: z.string().trim().min(1).max(512),
  mime_type: z.string().trim().min(1).max(100),
  storage_provider: z.enum(["cloudflare", "aws", "tencent"]).default("cloudflare"),
});

export const updateCompanyDocumentSchema = z.object({
  document_type: z.string().trim().min(2).max(100).nullable().optional(),
  document_name: z.string().trim().min(2).max(150).optional(),
}).refine((data) => Object.keys(data).length > 0, {
  message: "Minimal satu field harus diisi untuk update",
});

export const listCompanyDocumentQuerySchema = z.object({
  page: z.coerce.number().int().positive().default(1),
  limit: z.coerce.number().int().positive().max(100).default(10),
  document_type: z.string().trim().optional(),
  search: z.string().trim().optional(),
});

export const companyDocumentIdParamSchema = z.object({
  documentId: z.uuid("Format document_id tidak valid"),
});

export const companyDocumentCompanyIdParamSchema = z.object({
  companyId: z.uuid("Format company_id tidak valid"),
});
